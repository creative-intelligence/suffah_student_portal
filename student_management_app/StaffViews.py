...
def staff_add_resource(request):
    subjects = Subjects.objects.filter(staff_id=request.user.id)
    courses = Courses.objects.all()
    session_years = SessionYearModel.objects.all()
    context = {
    ...
    return render(request, "staff_template/add_resource_template.html", context)

def staff_add_resource_save(request):
    if request.method != "POST":
        messages.error(request, "Invalid Method")
        return redirect('staff_add_assignment')
    else:
        ...
        try:
            result = ClassResources(course_id=course_obj, subject_id=subject_obj, resource_name=resource_name, file_link=profile_pic_url, file_name = filename)
            result.save()
